...
export default class ScienceScreen extends Component {
  ...
  getNews = async () => {
    ...
    var url = 'https://newsapi.org/v2/top-headlines?country=in&category=science&apiKey=a1cacd357bb146d2a946022b95be617b';
    return fetch(url)
  ...
  render() {
    if (this.state.article === '') {
      return (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#004282' }}>
          <LottieView
        ...
        <NativeBaseProvider>
          ...
          >
            ...
            >
                ...
                renderItem={({ item, index }) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <Text style={tailwind("text-base text-center font-bold")}>{item.title}</Text>
                      <Text style={tailwind("font-semibold text-sm text-center text-blue-300")}>{item.description}</Text>
                    </Pressable>
                  </Box>
                )}
              ...
              onPress={() => {
                Speech.speak(
                  this.state.article.articles[this.state.index].title + this.state.article.articles[this.state.index].description
                )
              }
              }
